 ...
 *  full app cartesian). Public slug is an app-attribute word; internal
 *  attribution is source=platform / medium=<medium> / campaign=<key>.
 *  `medium` defaults to "creator"; set it to "friend" (or similar) on the entry
 *  to roll a whole group up separately from paid/partner creators. */
function creatorMedium(entry) {
  return entry.medium || 'creator';
}
    ...
    for (const promo of c.promos || []) {
      ...
      out.push({
        ...
        p: {
          ...
          label: `${medium === 'friend' ? 'Friend' : 'Creator'}: ${c.label || key} · ${c.assignedTo || 'unassigned'} · ${platform}`,
        },
      });
    }
  }
...
<!-- Open Graph / Twitter: makes a pasted link unfurl with a large preview card
     on WhatsApp, Reddit, Discord, Substack, Medium, X, etc. Unfurl bots read
     these static tags; they don't run the JS redirect, so humans still get sent
     on. /go/ is intentionally NOT blocked in robots.txt (only noindex) so these
     scrapers (facebookexternalhit etc.) can fetch the card. -->
...
// ---- registry doc (docs/link-registry.md) ----------------------------------

/** Human-readable ledger of every generated link: the link, what it is for, and
 *  who is holding it. Generated from link-config.json so it can never drift
 *  from the links that actually exist. */
function registryDoc(rows) {
  const md = (s) => String(s ?? '').replace(/\|/g, '\\|');
  const short = (u) => u.replace(/^https?:\/\//, '');
  const individuals = rows.filter((r) => r.kind === 'individual');
  const channels = rows.filter((r) => r.kind === 'placement');
  const assigned = individuals.filter((r) => r.entry.assignedTo);

  const peopleTable = (medium, verb) => {
    const list = individuals.filter((r) => r.p.utm_medium === medium);
    if (!list.length) return '_None yet._\n';
    return [
  ...
  const channelTables = Object.keys(apps).map((appKey) => [
    `### ${apps[appKey].name}`,
    '',
    '| Link | Purpose |',
    '| --- | --- |',
    ...channels.filter((r) => r.appKey === appKey)
      .map((r) => `| \`${md(short(r.shortUrl))}\` | ${md(r.p.label)} |`),
    '',
  ].join('\n')).join('\n');
...
${rows.length} links · ${individuals.length} held by people (${assigned.length} given out,
...
${peopleTable('friend', 'friend')}
## Links given to creators

${peopleTable('creator', 'collab')}
...
| Link | Purpose |
| --- | --- |
| \`${short(baseUrl)}/go/r?a=<app>&utm_source=…&utm_campaign=…\` | Build any one-off tracked link without adding it to the config. |
${Object.keys(apps).map((k) => `| \`${short(baseUrl)}/go/${k}?c=<campaign>\` | Short endpoint used by ${apps[k].name}'s own in-app share buttons. |`).join('\n')}
...
function build() {
  ...
  for (const appKey of Object.keys(apps)) {
    ...
    for (const code of Object.keys(placements)) {
      ...
      rows.push({ appKey, code, shortUrl, label: p.label, kind: 'placement', p });
    }
  }
  ...
  for (const promo of creatorPromos()) {
    const { appKey, code, p, entry } = promo;
    const app = apps[appKey];
    const slug = `${appKey}-${code}`;
    const shortUrl = `${baseUrl}/go/${slug}`;
    const qrSvg = qrAvailable ? qrEncode(shortUrl) : null;
    ...
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, 'index.html'), html);
    rows.push({ appKey, code, shortUrl, label: p.label, kind: 'individual', p, entry });
  }

  writeFileSync(join(OUT_DIR, 'index.html'), registryPage(rows));
  mkdirSync(join(ROOT, 'docs'), { recursive: true });
  writeFileSync(join(ROOT, 'docs', 'link-registry.md'), registryDoc(rows));
